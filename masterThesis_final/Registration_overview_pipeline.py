...
static_name = '210719_PR010_Overview_10-55-03_downscaled.tif'
moving_name = 'PR010_smaller_subset_version2.h5'
static_img = io.imread(os.path.join(path, static_name))
moving_img = h5py.File(os.path.join(path, moving_name), 'r')[u"/t00000/s00/5/cells"]
...
path = '/media/muellerg/Data SSD/Gesine/Data/Registration_Overview_Detailed/Gesine Pack_1209/'
static_data, static_affine, static_image = load_nifti(pjoin(path, '210719_PR010_Overview_10-55-03_downscaled.tif.nii.gz'), return_img=True)
static = static_data
moving_data, moving_affine, moving_img = load_nifti(pjoin(path, 'PR010_smaller_subset_version2.h5_00_5.nii.gz'), return_img=True)
moving = moving_data
