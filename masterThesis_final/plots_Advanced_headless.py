...
def plot_directionalityPolar(patch_size, data_l, data_r, method, name, path_output):
    ...
    color = [ "","#5790fc", "#e42536", "#f89c20", "#964a8b", "#656364"]
        #cm.cividis(np.linspace(0, 1, 6))
    for i in (data_l.keys()[1:]):
        freq_l = data_l[i] / np.sum(data_l[i])
        freq_r = data_r[i] / np.sum(data_r[i])
        ax1.plot(theta, np.array(freq_l), color = color[i])
        ax2.plot(theta, np.array(freq_r), color = color[i])
        max_lim = np.max(freq_l)
        if max_lim > x_lim:
            x_lim = max_lim
    title = 'Normalized orientation distributions'
    ...
    ax2.set_title('Right ACx', fontsize=16)
    ax1.set_title('Left ACx', fontsize=16)
    ax2.legend(['L1','L2/3', 'L4', 'L5','L6'], fontsize=14, loc=2, bbox_to_anchor=(1, 1))
    ...
    plt.tight_layout()
    save_name = 'PolarLayer_'+name+str(patch_size) +'_'+ method+'.png'
...
def plot_color2D_layerTonotopy(data, path_output, patch_size, side, annot, cmap = 'Oranges', pixel = 0.5417):
    ...
    if annot == True:
        plt.savefig(path_output + 'DiffFijiOriJ_Layers_tonotopy_' + side + '_'+ str(patch_size)  + 'annot.png', dpi=180)
    else:
        plt.savefig(path_output + 'DiffFijiOriJ_Layers_tonotopy_' + side + '_'+ str(patch_size)  + '.png', dpi=180)
    plt.close()
