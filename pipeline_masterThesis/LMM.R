...
data <- read.csv("~/Studium/MasterCMS/MasterThesis/DataPC/Result_92_12141718/Result_Fiji_92.csv")
...
data_L23<-data[!(data$layer=="L1" | data$layer=="L6"| data$layer=="L5"| data$layer=="L4"),]
attach(data_L1)

#plots
ggplot(data_L1L6, aes(x=domDir)) + geom_density() + scale_color_grey() + theme_classic()
ggplot(data_L1[!(data_L1$side=="l"),], aes(x=domDir)) + geom_histogram(aes(y=..density..), colour="black", fill="white") +
  geom_density(alpha=.2, fill="#FF6666") 
plot(domDir ~ side, data = data_L1)
ggplot(data_L1L6,aes(x=side,y=domDir)) + geom_point() + geom_smooth(method = "lm")

# 2. simple linear regression
fit_lm0 = lm(domDir~side, data = data_L1)
summary(fit_lm0)
anova(fit_lm0)
'ggplot(data,aes(x=side,y=domDir)) + geom_point() + geom_smooth(method = "lm")
abline(lm(domDir ~ side, data = data), col = "blue")
conf_interval <- predict(lm(domDir ~ side, data = data), 
...
ggplot(data,aes(x=side,y=domDir)) + geom_smooth(method = "lm",level = 0.95) + 
  geom_point() + facet_wrap(~sampleID + side, nrow = 4, ncol = 4)'

fit_lm1 = lm(domDir~side + layer, data = data_L1)
summary(fit_lm1)
anova(fit_lm1)
...
fit_lmer0 = lmer(domDir ~ side + (1 | sampleID), REML = FALSE, data_L1)
fit_lmer1 = lme(domDir~side, data=data_L1, random = ~1 | sampleID, method = "ML")
summary(fit_lmer0)
anova(fit_lmer0)
summary(fit_lmer1)
anova(fit_lmer1)
...
fit_lmer2 = lmer(domDir ~ side + (side | sampleID), REML = FALSE, data)
summary(fit_lmer2)
anova(fit_lmer2)
...
Null-model'
fit_lmer5 <- lmer(domDir ~ 1 + (1 | sampleID), data, REML = FALSE)
summary(fit_lmer5)
anova(fit_lmer5)
...
fit.dataL1_IO = bpnme(pred.I = domDir ~ side_num + (1|sampleID),
                      data = subdata,
...
# read in files 
load(file = "C:/Users/Gesine/Documents/Studium/MasterCMS/MasterThesis/DataPC/fit.subdata_IO-1000.rda")
load(file = "C:/Users/Gesine/Documents/Studium/MasterCMS/MasterThesis/DataPC/fit.subdata_1p-1000.rda")
load(file = "C:/Users/Gesine/Documents/Studium/MasterCMS/MasterThesis/DataPC/fit.subdata_2p-1000.rda")
# test statistics
fit(fit.dataL1_2p)
# coefficients of the fixed effects model
#categorical variable type: posterior estimates of the circular mean in deg -> 
#check whether there is an influence on outcome
NISTradianTOdeg(fit.dataL1_2p$circ.coef.means)
# Variance of the random effects in deg 
NISTradianTOdeg(fit.dataL1_2p$circ.res.varrand)