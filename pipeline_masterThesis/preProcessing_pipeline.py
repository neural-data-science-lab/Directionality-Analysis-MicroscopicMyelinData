...
def PreProcessing_Masks(side, batch_size, path_input, path_output, data_myelin, data_autofluorescence):
    '''
    ...
    :param batch_size: size of substack to avoid memory issues (default = 50 for PC, 5 for laptop)
    :param path_input: path to where the detailed datasets lie
    ...
    otsu_mask = []
    cortex_mask = []
    for batch in range(0, int(myelin.shape[0] / batch_size)):
        data = myelin[batch * batch_size:batch * batch_size + batch_size, :, :]
        autofl = autofluorescence[batch * batch_size:batch * batch_size + batch_size, :, :]
        g_autofl = gaussian_filter(autofl, sigma=45, mode='nearest')
        g_filter = gaussian_filter(data, sigma=2.0, mode='nearest')
        for z in range(data.shape[0]):  # ToDo: due to memory error -> possibly change to one-line for PC
            bg_filter.append(white_tophat(g_filter[z], disk(radius=35)))
            frangi_data.append(frangi(bg_filter[batch*data.shape[0]+ z], black_ridges=False))
            otsu_mask.append(bg_filter[batch*data.shape[0]+ z] > threshold_otsu(bg_filter[batch*z]))
            cortex_mask.append(g_autofl[z] > threshold_triangle(g_autofl[z]))
    ...
    # save finished dataset to output path: next step -> directionality (fiji ot OrientationJ)
    imsave(path_output+"C03_frangi.tif", frangi_data.astype('uint16'))
    imsave(path_output+"C03_otsu.tif", otsu_mask.astype('uint16'))
    imsave(path_output+"C03_cortex.tif", cortex_mask.astype('uint16'))
...
def OrientationJ(patch, sigma=2):
    ...
    (comparable to the OrientationJ implementation: http://bigwww.epfl.ch/demo/orientation/)
...
def directionality_layer_tonotopy(name_otsu, name_cortex, path, path_directionality, patch_size, colnames, header,
    ...
    for batch in range(0, int(depth / batch_size)):
        orientations = []
        dists3D = distance_transform_edt(mask_cortex[batch * batch_size:batch * batch_size + batch_size, :, :],
                                                 sampling=[8, 1, 1], return_distances=True)

        for z in range(batch_size):
            sx = sobel(dists3D[z], axis=0, mode='nearest')
            sy = sobel(dists3D[z], axis=1, mode='nearest')
...
def statistics2D(name_cortex, name_otsu, path, path_directionality, patch_size, colnames, header = True, slice=0):
    '''
    ...
    sx = sobel(distances, axis=0, mode='nearest')
    sy = sobel(distances, axis=1, mode='nearest')
    sobel = np.arctan2(sy, sx) * 180 / np.pi
    orientations = gaussian_filter(sobel, sigma=2)
    max_dist = 750.0 / 0.542
...
def plot_color2D_layerTonotopy(stats, nbr, save_path, patch_size, method, cmap = 'PuOr', pixel = 0.542):
    ...
    def plot_domOrientation(name_data, path, statistics, patch_size, save_path, slice=0):  # cividis, PuOr,
...
side = 'Left'   #ToDo: incorporate later
...
####################################################### main #########################################################
frangi_data, otsu_mask, cortex_mask = PreProcessing_Masks(side, batch_size, path_input, path_output, data_myelin, data_autofluorescence)

# ToDo: have key whether Fiji directionality or orientationJ in argparse -> no read in ov orientationJ csv needed
