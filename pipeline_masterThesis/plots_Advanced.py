...
def plot_color2D_layerTonotopy(data, path_output, patch_size, side, cmap = 'Oranges', pixel = 0.5417):
    '''
    ...
    sns.heatmap(data, cmap=cmap, square=True, xticklabels=False, yticklabels=False,
                vmin=0, vmax=90, center=0, cbar_kws={"shrink": .6}, annot=True, annot_kws={"size": 5}) #
    ...
    plt.savefig(path_output +'DifferenceFijiOriJ_Layers_tonotopy_'+side+str(patch_size)+'.png', dpi=180)
    plt.close()
...
for i in range(len(method)):
    result_left = pd.read_csv(path + 'Result_'+ side[0] + str(patch_size) + '_'+ method[i] + '.csv', encoding = "ISO-8859-1")
    result_right = pd.read_csv(path + 'Result_'+ side[1] + str(patch_size) + '_'+ method[i] + '.csv', encoding = "ISO-8859-1")
    distribution_left = pd.read_csv(path + 'Distribution_'+ side[0] + str(patch_size) + '_'+ method[i] + '.csv', encoding = "ISO-8859-1")
    distribution_right = pd.read_csv(path + 'Distribution_'+ side[1] + str(patch_size) + '_'+ method[i] + '.csv', encoding = "ISO-8859-1")
    d_l, nbr_l = layer(result_left, distribution_left, layers)
    d_r, nbr_r = layer(result_left, distribution_right, layers)
    plot_directionalityPolar(patch_size, d_l, nbr_l, d_r, nbr_r, method[i], path)
    plot_nbrPatchesInCortex(patch_size, nbr_l, nbr_r, method[i], path)


# absolute difference between Fiji_directionality and OrientationJ orientations (dominant direction)
for i in range(len(side)):
    data_frangi = side[i] + '_C03_frangi.tif'
    frangi_data = io.imread(os.path.join(path, data_frangi))
    result_left = pd.read_csv(path + 'Result_'+ side[i] + str(patch_size) + '_'+ method[0] + '.csv', encoding = "ISO-8859-1")
    result_right = pd.read_csv(path + 'Result_'+ side[i] + str(patch_size) + '_'+ method[1] + '.csv', encoding = "ISO-8859-1")
    ...
    plot_color2D_layerTonotopy(data, path, patch_size, side[i], cmap = 'RdGy')
